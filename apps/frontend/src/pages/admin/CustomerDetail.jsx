...
export default function CustomerDetail() {
  ...
  const handleDelete = async () => {
    if (confirm(t('customersPage.deleteConfirm'))) {
  ...
  if (!data?.customer) {
    return <div className="p-12 text-center text-muted-foreground">{t('customersPage.notFound')}</div>;
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">{formatPhoneNumber(customer.phone) || t('customersPage.noPhone')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-card p-6 rounded-2xl border border-surface-border">
          <h3 className="text-sm font-medium text-muted-foreground">{t('customersPage.totalDebtLabel')}</h3>
        ...
        <div className="bg-card p-6 rounded-2xl border border-surface-border flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-medium text-muted-foreground">{t('customersPage.clientSince')}</h3>
      ...
      <div className="bg-card rounded-2xl border border-surface-border overflow-hidden">
        <div className="px-6 py-4 border-b border-surface-border">
          <h2 className="text-lg font-semibold text-text-primary">{t('customersPage.transactionHistory')}</h2>
        </div>
        <div className="divide-y divide-surface-border">
          {customer.transactions.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">{t('customersPage.noTransactions')}</div>
          ) : (
            [...customer.transactions].reverse().map((tx, index) => (
              <div key={index} className="px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-medium text-text-primary">
                      {tx.type === 'increase' ? t('customersPage.debtIncrease') : t('customersPage.repayment')}
                    </p>
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5 tabular-nums">{t('customersPage.balance')}: {formatMoney(tx.newTotalDebt).replace(' MRU', '')}</p>
                </div>
              </div>
            ))
          )}
